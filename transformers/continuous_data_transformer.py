...
class ContinuousDataTransformer():
	...
	def transform(self, column_data):
		...
		"""
		# Move data to GPU
		column_data = cp.asarray(column_data)
		column_data = (column_data - self.mean) / self.std  # Normalize data
		n_samples = len(column_data)

		# Compute means and standard deviations for the Gaussian components
		means, covariances = self.model.get_component_stats()
		means = cp.asarray(means).reshape((1, self.n_clusters))  # Move to GPU and reshape
		stds = cp.sqrt(cp.asarray(covariances)).reshape((1, self.n_clusters))  # Move to GPU and compute sqrt

		# Normalize the data
		features = (column_data[:, None] - means) / (4 * stds)  # Shape: (n_samples, n_clusters)
		features = features.reshape(len(column_data), self.n_clusters) 

		# Use GPU for active components
		...
		probs = cp.asarray(self.model.predict_proba(cp.asnumpy(column_data)))  # Convert to CuPy array
		#probs = cp.squeeze(probs, axis=1)  # Remove unnecessary dimensions
		probs = probs[:, active_components]  # Filter for active components

		# Normalize probabilities
		probs += 1e-6  # Avoid zero probabilities
		probs /= cp.sum(probs, axis=1, keepdims=True)

		# Weighted sampling using CDF (GPU implementation)
		cdf = cp.cumsum(probs, axis=1)  # Cumulative distribution function
		random_vals = cp.random.rand(n_samples).reshape(-1, 1)  # Random values for sampling
		opt_sel = cp.sum(random_vals > cdf, axis=1)  # Sample indices based on CDF

		# Create one-hot encoding for the selected modes
		probs_onehot = cp.zeros_like(probs)
		probs_onehot[cp.arange(n_samples), opt_sel] = 1

		# Obtain normalized values for the selected mode
		features = features[:, active_components]  # Shape: (n_samples, n_active_components)
		selected_features = features[cp.arange(n_samples), opt_sel].reshape(-1, 1)  # Select based on opt_sel
		selected_features = cp.clip(selected_features, -0.99, 0.99)  # Clip values

		# Reorder one-hot encodings based on frequency
		mode_frequencies = cp.sum(probs_onehot, axis=0)
		reordered_indices = cp.argsort(-mode_frequencies)  # Sort in descending order
		reordered_probs_onehot = probs_onehot[:, reordered_indices]

		# Save ordering for inverse transformation (convert back to NumPy for compatibility)
		self.ordering.append(cp.asnumpy(reordered_indices))

		# Combine selected features and reordered one-hot encodings
		transformed_data = cp.hstack([selected_features, reordered_probs_onehot])  # Shape: (n_samples, output_dim)
